...
My animation shows how an NFL team is chosen step by step. It starts with the NFL, then moves to the NFC conference. Next, it shows the NFC West division and the teams in it. The program selects the San Francisco 49ers. The final scene displays some of the main 49ers players using data from 2D arrays.

## Image Filter Analysis

One filter used is the keepColor filter. It keeps one color channel and removes the others, like keeping only red. This makes the image look tinted in that color. Another filter used is motionBlur, which averages nearby pixels to create a blur effect. This is used on the NFL logo to make the intro more dynamic.